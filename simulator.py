...
# event types
ARRIVAL = 'A'  # pair of tensors arrives at the service
DEPARTURE = 'D'  # product leaves the service
...
service_capacity: how many operations can be done at the same time
queue_capacity: how many pairs of tensors can wait in queue at the same time
arrival_rate: average number of pairs of tensors that arrive per time unit (lambda)
"""


def service_simulator(simulation_time, arrival_rate, service_rate, seed_x, seed_c, queue_capacity = 15):
    current_time = 0
    events = []  # list of (event_type, event_time) tuples, sorted by event time
    queue_size = 0  # amount of pairs of tensors currently waiting in queue (queue_size <= queue_capacity)

    # interest measures
    arrival_count = 0  # pairs of tensors who arrived
    departure_count = 0  # pairs of tensors who were operated and left
    drop_count = 0  # pairs of tensors who left due to queue being full
...
# print(service_simulator(simulation_time = 3600, arrival_rate = float(1/Ec), service_rate = float(1/Ex)))
